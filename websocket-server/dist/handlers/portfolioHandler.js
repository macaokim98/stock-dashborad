...
exports.PortfolioHandler = void 0;
...
class PortfolioHandler {
    ...
    subscribeToPortfolio(socket) {
        if (this.subscribedClients.has(socket.id)) {
            logger_1.logger.debug(`Client ${socket.id} already subscribed to portfolio`);
            return;
        }
        ...
        logger_1.logger.info(`Client ${socket.id} subscribed to portfolio updates`);
    }
    unsubscribeFromPortfolio(socket) {
        if (!this.subscribedClients.has(socket.id)) {
            return;
        }
        ...
        logger_1.logger.info(`Client ${socket.id} unsubscribed from portfolio updates`);
    }
    startPortfolioUpdates() {
        ...
        logger_1.logger.info(`Portfolio updates started (${updateInterval}ms interval)`);
    }
    stopPortfolioUpdates() {
        if (this.portfolioUpdateInterval) {
            ...
            logger_1.logger.info('Portfolio updates stopped');
        }
    }
    broadcastPortfolioUpdate() {
        ...
        logger_1.logger.debug(`Broadcasted portfolio update to ${this.subscribedClients.size} clients`);
    }
...
//# sourceMappingURL=portfolioHandler.js.map