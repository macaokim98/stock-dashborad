...
exports.RealTimeStockHandler = void 0;
const logger_1 = require("../utils/logger");
class RealTimeStockHandler {
    constructor(stockDataStreamer) {
    ...
    setupEventListeners() {
        ...
        this.stockDataStreamer.on('stock_update', (quote) => {
            this.broadcastStockUpdate(quote);
        });
        // Listen for market data updates
        this.stockDataStreamer.on('market_data', (marketData) => {
            this.broadcastMarketData(marketData);
        });
    }
    subscribeToStocks(socket, symbols) {
        ...
        validSymbols.forEach(symbol => {
            if (!currentSubscriptions.has(symbol)) {
                ...
                logger_1.logger.debug(`Client ${socket.id} subscribed to ${symbol}`);
            }
        });
        ...
        logger_1.logger.info(`Client ${socket.id} subscribed to stocks: ${validSymbols.join(', ')}`);
    }
    unsubscribeFromStocks(socket, symbols) {
        if (!Array.isArray(symbols) || symbols.length === 0) {
            socket.emit('error', { message: 'Invalid symbols array' });
            return;
        }
        ...
        validSymbols.forEach(symbol => {
            if (currentSubscriptions.has(symbol)) {
                ...
                logger_1.logger.debug(`Client ${socket.id} unsubscribed from ${symbol}`);
            }
        });
        ...
        logger_1.logger.info(`Client ${socket.id} unsubscribed from stocks: ${validSymbols.join(', ')}`);
    }
    ...
    broadcastStockUpdate(quote) {
        ...
        logger_1.logger.debug(`Broadcasted ${quote.symbol} update to room ${room}: $${quote.price} (${quote.changePercent > 0 ? '+' : ''}${quote.changePercent}%)`);
    }
    broadcastMarketData(marketData) {
        // Broadcast market data to all connected clients
        const io = require('../server').io;
        io.emit('market_overview', marketData);
        logger_1.logger.debug(`Broadcasted market data update to all clients`);
    }
...
exports.RealTimeStockHandler = RealTimeStockHandler;
//# sourceMappingURL=realTimeStockHandler.js.map