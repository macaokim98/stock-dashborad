...
export class NotificationHandler {
  ...
  private clientAlerts: Map<string, Set<string>> = new Map();

  subscribeToNotifications(socket: Socket) {
    if (this.subscribedClients.has(socket.id)) {
      logger.debug(`Client ${socket.id} already subscribed to notifications`);
      return;
    }
    ...
    logger.info(`Client ${socket.id} subscribed to notifications`);
  }
  ...
  setPriceAlert(socket: Socket, data: {
    ...
    logger.info(`Price alert created: ${socket.id} - ${symbol} ${condition} $${targetPrice}`);
  }

  removePriceAlert(socket: Socket, alertId: string) {
    ...
    logger.info(`Price alert removed: ${socket.id} - ${alertId}`);
  }

  checkPriceAlerts(symbol: string, currentPrice: number) {
  ...
  sendNotification(clientId: string, notification: Notification) {
    const io = require('../server').io;
    
    // Send to specific client
    io.to(clientId).emit('notification', notification);
    
    logger.debug(`Notification sent to ${clientId}: ${notification.title}`);
  }

  broadcastMarketNews(title: string, message: string, data?: any) {
    ...
    logger.info(`Market news broadcasted: ${title}`);
  }
  ...
  private getAlertsBySymbol(alerts: PriceAlert[]) {
