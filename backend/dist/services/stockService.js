...
const alphaVantageService_1 = require("./alphaVantageService");
class StockService {
    constructor() {
        this.yfinanceService = new yfinanceService_1.YFinanceService();
        this.alphaVantageService = new alphaVantageService_1.AlphaVantageService();
    }
    async getMarketOverview() {
        try {
            logger_1.logger.info('Fetching market overview from YFinance');
            ...
            const indices = indexMapping.map(({ symbol, name }) => {
                ...
                if (data) {
                    ...
                    logger_1.logger.warn(`No data available for ${symbol}, using fallback`);
                    return {
                        name,
                        symbol: symbol.replace('^', ''),
                        value: 0,
            ...
            logger_1.logger.info(`Successfully fetched market overview: ${indices.length} indices`);
            return indices;
        }
        catch (error) {
    ...
    async getStockQuote(symbol) {
        try {
            logger_1.logger.info(`Fetching quote for ${symbol} from YFinance`);
            // Try YFinance first
            ...
            if (yfinanceData) {
                ...
                logger_1.logger.info(`Successfully fetched quote from YFinance for ${symbol}: $${stockQuote.price}`);
                return stockQuote;
            }
            // Fallback to AlphaVantage if YFinance fails
            logger_1.logger.info(`YFinance failed for ${symbol}, trying AlphaVantage as fallback`);
            ...
            if (alphaVantageData) {
                ...
                logger_1.logger.info(`Successfully fetched quote from AlphaVantage for ${symbol}: $${stockQuote.price}`);
                return stockQuote;
            }
            logger_1.logger.warn(`No data found for ${symbol} from both YFinance and AlphaVantage`);
            return null;
        }
        catch (error) {
    ...
    async getStockHistory(symbol, interval, range) {
        try {
            logger_1.logger.info(`Fetching historical data for ${symbol} (${range}, ${interval})`);
            ...
            const historicalData = await this.yfinanceService.getHistoricalData(symbol, yfinancePeriod, yfinanceInterval);
            const history = historicalData.map(data => ({
                date: data.date,
                open: data.open,
                high: data.high,
                low: data.low,
            ...
            logger_1.logger.info(`Successfully fetched historical data for ${symbol}: ${history.length} points`);
            return history;
        }
        catch (error) {
            logger_1.logger.error(`Error fetching history for ${symbol}:`, error);
            throw new Error('Failed to fetch stock history');
        }
    }
    async searchStocks(query) {
        try {
            logger_1.logger.info(`Searching stocks for query: ${query}`);
            const searchResults = await this.yfinanceService.searchSymbols(query);
            if (searchResults.length === 0) {
                logger_1.logger.info(`No search results found for: ${query}`);
                return [];
            }
            ...
            for (const symbol of symbols) {
                try {
                    ...
                    logger_1.logger.warn(`Failed to get quote for ${symbol} during search`);
                }
            }
            logger_1.logger.info(`Search completed for ${query}: ${quotes.length} results`);
            return quotes;
        }
        catch (error) {
    ...
    async getTopGainers() {
        try {
            logger_1.logger.info('Fetching top gainers');
            ...
            for (const symbol of symbols) {
                try {
                    ...
                    logger_1.logger.warn(`Failed to get quote for ${symbol} in top gainers`);
                }
            }
            const gainers = quotes
                ...
                .slice(0, 5);
            logger_1.logger.info(`Top gainers fetched: ${gainers.length} stocks`);
            return gainers;
        }
        catch (error) {
    ...
    async getTopLosers() {
        try {
            logger_1.logger.info('Fetching top losers');
            ...
            for (const symbol of symbols) {
                try {
                    ...
                    logger_1.logger.warn(`Failed to get quote for ${symbol} in top losers`);
                }
            }
            const losers = quotes
                .filter(stock => stock.changePercent < 0)
                ...
                .slice(0, 5);
            logger_1.logger.info(`Top losers fetched: ${losers.length} stocks`);
            return losers;
        }
        catch (error) {
    ...
    async getChartData(symbol, period = '1d', interval = '1m', startDate, endDate) {
        try {
            ...
            if (startDate && endDate) {
                ...
                try {
                    historicalData = await this.yfinanceService.getHistoricalData(symbol, yfinancePeriod, yfinanceInterval);
                    ...
                    logger_1.logger.warn(`YFinance failed for chart data, trying AlphaVantage fallback for ${symbol}`);
                    ...
                    if (yfinanceInterval === '1m' || yfinanceInterval === '5m' || yfinanceInterval === '15m' || yfinanceInterval === '30m' || yfinanceInterval === '1h') {
                        ...
                        const alphaData = await this.alphaVantageService.getIntradayData(symbol, alphaInterval);
            ...
            logger_1.logger.info(`Successfully fetched chart data for ${symbol}: ${chartData.length} points`);
            return chartData;
        }
        catch (error) {
            logger_1.logger.error(`Error fetching chart data for ${symbol}:`, error);
